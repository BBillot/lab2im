...
# third-party imports
from ext.neuron.layers import Resize
import ext.neuron.models as nrn_models

# project imports
import utils
import building_blocks
from volume_editting import edit_volume
from deep_learning_tools import evaluate
...
def predict(path_images,
    ...
    label_list, _ = utils.get_list_labels(path_label_list=path_segmentation_label_list, FS_sort=True)
    ...
    for idx, (im_path, seg_path, posteriors_path) in enumerate(zip(images_to_segment, path_segmentations, path_posteriors)):
...
def preprocess_image(im_path, n_levels, crop_shape=None, padding=None):
    ...
    if crop_shape is not None:
        ...
        im = edit_volume.crop_volume_with_idx(im, crop_idx=crop_idx)
    else:
        crop_idx = None
...
def preprocessing_model(resample, model_input_shape, header, n_channels, n_dims, n_levels):
    ...
    resampled = Resize(zoom_factor=resample_factor_corrected, name='pre_resample')(im_input)
...
def postprocessing_model(unet, posteriors_patch_shape, resample, sigma_smoothing, n_dims):
    ...
    if resample is not None:
        last_tensor = Resize(size=posteriors_patch_shape[:-1], name='post_resample')(last_tensor)
    ...
    if sigma_smoothing != 0:
        ...
        kernels_list = building_blocks.get_gaussian_1d_kernels(sigma_smoothing)

        # blur each image channel separately
        last_tensor = building_blocks.blur_tensor(split[0], kernels_list, n_dims)
        for i in range(1, n_labels):
            temp_blurred = building_blocks.blur_tensor(split[i], kernels_list, n_dims)
